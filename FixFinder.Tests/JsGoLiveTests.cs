...
/// <summary>
/// JavaScript and Go programs broken the way learners break them, run for real through Node and <c>go run</c> - each must end
/// with its rule's fix, checked and ready to copy.
/// </summary>
/// <remarks>Cases skip when Node or Go is not installed, and a skip looks like a pass.</remarks>
public class JsGoLiveTests
{
    private static string Go(string body, string imports = "import \"fmt\"\n\n", string extra = "") =>
        "package main\n\n" + imports + extra + "func main() {\n" + body + "}\n";

    /// <summary>The runtime, the construct, the file name, the program, the rule, and text the copied fix must contain.</summary>
