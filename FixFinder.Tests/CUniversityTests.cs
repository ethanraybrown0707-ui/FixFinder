...
/// <summary>
/// The C mistakes of later years - macros, pointers, multi-dimensional arrays, input - each rule's fix and
/// refusals from gcc's, MSVC's and AddressSanitizer's words, then the same programs built for real.
/// </summary>
/// <remarks>Live cases skip when their toolchain is missing, and a skip looks like a pass.</remarks>
public class CUniversityTests : IDisposable
{
    ...
    /// <summary>No main anywhere - the linker names no file, so the one function a letter from main is found in the project.</summary>
    [Theory]
    ...
    /// <summary>The toolchain, the construct, the program, typed input, the rule, and text the copied fix must contain.</summary>
    public static TheoryData<string, string, string, string?, string, string> Constructs
    {
        get
        {
            ...
            // Only MSVC's C runtime dies writing to address 0; MinGW's scanf quietly refuses the null pointer.
