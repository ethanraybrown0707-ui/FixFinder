...
/// <summary>
/// Covers the response cache, which is what keeps development off the network.
/// </summary>
/// <remarks>
/// Two of these matter more than the rest. The credential test is a security property: the
/// Stack Exchange key travels in the query string, so a cache that stored URLs verbatim would
/// write it to disk in plain text. The CacheOnly test is the one that makes the switch in the window
/// trustworthy - if it ever silently fell through to the network, every promise made about
/// quota safety would be false.
/// </remarks>
public class HttpCacheTests
{
    ...
    /// <summary>
    /// The Stack Exchange app key is a query parameter, so the URL itself is a secret.
    /// </summary>
    [Fact]
    ...
    /// <summary>Adding a key changes the allowance, never the results - so it must not evict the cache.</summary>
    [Fact]
    ...
    /// <summary>Builds a client whose transport counts calls and can be told what to return.</summary>
    ...
    /// <summary>
    /// Caching a rate-limit refusal would lock the tool out for the whole TTL, and it would look
    /// exactly like a real answer while it did so.
    /// </summary>
    [Fact]
