...
/// <summary>
/// The Python mistakes of later years - classes, closures, async, comprehensions, collections - each
/// rule's fix and refusals, then the same programs run for real.
/// </summary>
/// <remarks>Live cases skip without Python 3.12 or later, and a skip looks exactly like a pass.</remarks>
public class PythonUniversityTests : IDisposable
{
    ...
    // ------------------------------------------------------------------ rules that read the line, not the underline
    ...
    /// <summary>The Microsoft Store Python keeps its standard library under WindowsApps, and a crash inside it is not the user's code.</summary>
    [Fact]
