...
/// <summary>
/// The rules that work a fix out from the code, read against real source files and real message shapes.
/// </summary>
/// <remarks>
/// None of these compile anything, so they run anywhere. What they pin down is what each rule
/// proposes and - mostly - what it refuses to propose, because a rule is only safe to have if it
/// declines everything it cannot be exact about. <see cref="LocalFixLiveTests"/> is where the
/// proposals meet a real compiler.
/// </remarks>
public class LocalFixTests : IDisposable
{
    ...
    /// <summary>The typos people actually make are swaps, and plain Levenshtein calls a swap two edits.</summary>
    [Theory]
    ...
    /// <summary>
    /// <c>printn</c> is one edit from print, println and printf alike. Choosing would be guessing.
    /// </summary>
    [Fact]
    ...
    /// <summary>Saying this right needs a no-newline marker on one side only, so it is not said at all.</summary>
    [Fact]
    ...
    /// <summary>
    /// The Microsoft Store build of Python cannot see files other programs write under LocalAppData.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The check that let four Python fixes through: the copy could not be found, the compiler said
    /// so in a way no parser reads, and "no errors" was taken as a pass.
    /// </summary>
    [Fact]
    ...
    /// <summary>A syntax error hides what comes after it, so fixing one may reveal the next.</summary>
    [Fact]
    ...
    /// <summary>Python 2's trailing comma meant "no newline"; rewriting it would change the output.</summary>
    [Theory]
    ...
    /// <summary>The underline Python 3.11+ prints is what pins the operand; this is a real one.</summary>
    [Fact]
    ...
    /// <summary>Declaring it on the method does nothing for a lambda body, so nothing is offered.</summary>
    [Fact]
    ...
    /// <summary>Index 5 in a 3-element array is some other bug; only index == length is a loop's &lt;=.</summary>
    [Theory]
    ...
    public void ALoopThatReachesTheLengthStopsOneBefore(string message, string? expected)
    {
        ...
        // A Java stack frame names the file and nothing else; it is found under the source root.
    ...
    /// <summary>The build succeeded; the warning is the only record of why the program then died.</summary>
    [Fact]
    ...
    /// <summary>A misspelt call in C is only a warning and a linker error, and only the warning has a line.</summary>
    [Fact]
    ...
    /// <summary>These used to be reported as "finished unhappily", which is how a crash went unlocated.</summary>
    [Theory]
    ...
    /// <summary>The compile-time form of a missing library, which is where an ordinary program meets it.</summary>
    [Fact]
