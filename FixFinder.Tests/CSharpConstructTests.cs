...
/// <summary>
/// The C# mistakes a beginner makes: each rule's fix and refusals from Roslyn's own messages, then the
/// same mistakes built for real by the .NET SDK.
/// </summary>
/// <remarks>
/// <b>The live cases need an SDK that runs a lone .cs file - 10 or later - and skip without one.</b>
/// A skip looks like a pass: a case that really ran takes seconds.
/// </remarks>
public class CSharpConstructTests : IDisposable
{
    ...
    /// <summary>A loop that runs while i &lt;= Length reads one past the end, and the loop is the fix - not the line that threw.</summary>
    [Theory]
    ...
    /// <summary>The program, the answer's id, and text the copied fix must contain.</summary>
