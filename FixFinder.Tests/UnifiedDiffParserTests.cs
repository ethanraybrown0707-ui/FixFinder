...
/// <summary>
/// Covers the diff parser, which is the last thing standing between a stranger's issue comment
/// and your source tree.
/// </summary>
/// <remarks>
/// The refusal tests carry as much weight as the success ones. A parser that quietly repairs a
/// malformed hunk produces a patch nobody wrote, and the whole safety story downstream assumes
/// that whatever comes out of here is exactly what the author put in.
/// </remarks>
public class UnifiedDiffParserTests
{
    ...
    /// <summary>
    /// An absent count means one, so this is a legal single-line hunk.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The marker belongs to the line above it, not to a line of its own.
    /// </summary>
    /// <remarks>
    /// Getting this wrong appends a newline the file never had. Silent, one byte, and it turns
    /// up in every later diff of that file.
    /// </remarks>
    [Fact]
    ...
    /// <summary>
    /// The single most common reason a good patch "will not parse".
    /// </summary>
    /// <remarks>
    /// An empty context line is a lone space, and every markdown renderer strips trailing
    /// whitespace - so by the time a diff has been through an issue comment, those lines are
    /// empty strings. Reading an empty line as the end of the hunk rejects a patch that is
    /// perfectly intact.
    /// </remarks>
    [Fact]
    public void EmptyContextLinesStrippedByMarkdownAreStillContext()
    {
        var mangled = Lines(
            ...
            "@@ -1,5 +1,5 @@",
            " import os",
            "",                       // was " ", trailing space eaten in transit
            "-x = 1",
            "+x = 2",
            "",                       // likewise
            " print(x)");
    ...
    /// <summary>
    /// A hunk whose header disagrees with its body has been truncated or hand-edited.
    /// </summary>
    /// <remarks>
    /// There is deliberately no repair path. Reconciling it would mean guessing which of the two
    /// is right, and that guess decides what gets written to a source file.
    /// </remarks>
    [Fact]
    ...
    /// <summary>
    /// The path traversal case, refused here as well as at apply time.
    /// </summary>
    /// <remarks>
    /// The parser does not resolve paths and must not: it strips git's a/ and b/ prefixes and
    /// nothing more, so the traversal survives intact for the containment check to catch. This
    /// test pins that it is neither silently normalised away nor quietly resolved.
    /// </remarks>
    [Fact]
