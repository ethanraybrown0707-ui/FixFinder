...
/// <summary>
/// The mistakes people make learning Python, one per topic of a beginner's course, and what FixFinder
/// does with each.
/// </summary>
/// <remarks>
/// Measured first: fifty programs, one classic mistake per topic, run through FixFinder. Eight came
/// back with a right fix, one with a wrong one (<c>age + 1</c> became <c>age + str(1)</c>, which
/// prints 181), and one with an unsafe one (<c>pip install maths</c> for a misspelt <c>math</c>). Two
/// more failed where Python had named the answer itself. The tests here pin the corrections to all
/// four, and the rules added for the thirteen mistakes whose message pins the answer down.
/// <para>
/// The unit tests read real message shapes and need nothing installed. The live ones run Python
/// 3.12 or later, and skip without it - which looks like a pass, so a live case that took single
/// milliseconds did not run.
/// </para>
/// </remarks>
public class PythonBeginnerTests : IDisposable
{
    ...
    /// <summary>The traceback Python prints for one line, with its underline.</summary>
    ...
    // ------------------------------------------------------------------ the wrong fix, corrected

    /// <summary>
    /// <c>age + 1</c> is arithmetic on text read by input(). Converting the 1 made it print "181".
    /// </summary>
    [Theory]
    ...
    public void WhichSideOfPlusToConvertDependsOnWhatIsOnTheLeft(string left, string right, string? expected)
    {
        ...
        // Python's shape for a binary operation: ~ under the left operand and its space, ^ under the
        // operator, ~ under the space and the right operand - built from the operands, not counted by hand.
    ...
    /// <summary>An underline that ends inside a name is refused, never used to wrap half of it.</summary>
    [Fact]
    ...
    // ------------------------------------------------------------------ syntax a beginner brings from elsewhere
    ...
    /// <summary>Closed where the line ends - but never when the next line looks like the rest of it.</summary>
    [Theory]
    ...
    /// <summary>Python does not suggest a name two swapped letters away, so the file's own names are the candidates.</summary>
    [Theory]
    ...
    public void ASwappedLetterIsMatchedToANameTheFileDefines(string source, string wrong, string? expected)
    {
        ...
        // pritn is corrected to print on its own line; the line checked here is the one with the typo.
        if (wrong == "pritn")
        {
    ...
    /// <summary>
    /// A NameError raised while handling another exception: its own frame is the except line, while
    /// the culprit frame - chosen from the root cause, for searching - is the line that raised first.
    /// </summary>
    [Fact]
    ...
    /// <summary>Topic, program, the rule that should answer, and what the copied fix contains.</summary>
    ...
    public async Task TheBeginnersMistakeIsFixed(string topic, string source, string id, string expected)
    {
        ...
        // Never alongside an offer to download a package for what was a typo.
    ...
    /// <summary>
    /// Near a standard module, but not a typo of it: lxml.etree is not math-shaped, and xml has no
    ...
    /// <summary>With nothing typed, it says so rather than searching for EOFError; typed, it reaches the real bug.</summary>
    [Fact]
